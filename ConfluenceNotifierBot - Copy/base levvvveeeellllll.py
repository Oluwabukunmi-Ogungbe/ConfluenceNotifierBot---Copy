            ...
            if await is_valid_buy_message(text):
                ...
                if trader_match and token_address:
                    ...
                    if token_address != EXCLUDED_TOKEN:
                        ...
                        # Construct and send a message for each valid buy
                        message_to_send = (
                            f"Trader {trader} bought {token_address}:\n"
                            f"Amount: {sol_amount:.1f} SOL\n"
                            f"Pump Type: {pump_type}\n"
                        )
                        if market_cap is not None:
                            message_to_send += f"Market Cap: ${market_cap['value']:.2f}{market_cap['unit']}"
                        
                        # Send the constructed message
                        await context.bot.send_message(chat_id=session.chat_id, text=message_to_send)

                        # Store trader information in session (if needed)
