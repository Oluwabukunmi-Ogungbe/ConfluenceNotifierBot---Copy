...
async def is_valid_buy_message(text):
    ...
    if 'CashCash_alert_bot' in chat_limits:
        return True
...
async def monitor_channels(context, session):
    ...
    while session.is_monitoring:
        ...
        for address, traders in session.multi_trader_tokens.items():
            if len(traders) >= 2:
                # Sort traders by timestamp to get the order of trades
                sorted_traders = sorted(
                    traders, 
                    key=lambda t: session.token_timestamps[address].get(t, 0)
                )

                # Construct message for multiple traders
                ...
                for idx, trader in enumerate(sorted_traders, 1):
                    ...
                    # Get the exact MC/MCP text from the original message
                    market_cap_match = ""
                    trader_message = (
                        f"{idx}{'st' if idx == 1 else 'nd' if idx == 2 else 'rd' if idx == 3 else 'th'} trader {trader} bought {sol_amount:.1f} SOL "
                        f"on {pump_type} at {market_cap_match}"
                    ).strip()
...
async def main():
    ...
    # Add command handlers for webhook mode
    application.add_handler(CommandHandler("start", start))
    application.add_handler(CommandHandler("stop", stop))

    logging.info("Starting bot with webhooks...")

    # Set up webhook URL (make sure to replace YOUR_DOMAIN with your actual domain)
    webhook_url = f"https://46d8-102-135-211-94.ngrok-free.app/{BOT_TOKEN}"
    
    # Start webhook for updates instead of using polling
    await application.initialize()  # Initialize the application before running webhook
